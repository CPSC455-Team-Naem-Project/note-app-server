...
interface UserNoteModel extends Model<IUserNote> {
  ...
  findPublicNotes(filteredObject: FilterObject): Promise<Document>;
...
UserNoteSchema.static(
  'removeNote',
  function removeNote(userId: string, noteId: string) {
...
UserNoteSchema.static(
  ...
  async function saveNoteToSavedNotes(note: IUploadedNote) {
    const data = await this.findByIdAndUpdate(
      note.userId,
...
UserNoteSchema.static('findPublicNotes', async function findPublicNotes(filteredObject) {
